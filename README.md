...
복수전공·심화전공 대학생이 자신의 수강 이력과 학번별 커리큘럼을 입력하면, 적합한 진로를 추천하고 선택한 진로 기준으로 다음 1~2개 학기 수강 계획까지 설계하는 AI 진로 설계 웹앱.
...
이 서비스는 수강 이력과 전공 조합을 구조화된 규칙으로 분석해 진로 적합도를 계산하고, 선택한 진로 기준으로 다음 1~2개 학기 수강 계획을 규칙 기반으로 제안한다. AI 설명 기능은 보조 기능이며, 핵심 추천/계획 흐름은 외부 LLM 없이도 동작한다.
...
| 과목 | 17개 (샘플 데이터 기반) |
| 계획 범위 | 다음 1~2개 학기, 12 / 15 / 18학점 옵션 |
...
/app
  ...
  /api/plan/route.ts        # POST — 규칙 기반 다음 학기 계획 생성
...
/lib
  ...
  planning.ts               # 학기별 수강 계획 엔진
  llm.ts                    # OpenAI 호출 + fallback 생성

/components
  IntakeForm.tsx            # 입력 폼 (select + 체크박스 + 데모 버튼)
  ResultCard.tsx            # 추천 결과 카드
  PlanSetupPanel.tsx        # 2단계 계획 옵션 입력
  SemesterPlanPanel.tsx     # 학기별 계획 결과
  RoadmapPanel.tsx          # AI 해설 + 로드맵 패널
```

추천 로직과 수강 계획 엔진은 LLM에 의존하지 않는다. 점수 계산과 과목 배치는 구조화 데이터 기반 규칙으로 처리하고, LLM은 결과 설명 문장 생성에만 사용한다.
...
서비스를 실행하면 `/recommend` 화면 상단에 빠른 데모 프로필 버튼 3개가 있다. 각 시나리오는 1단계 진로 추천 후, 결과 카드의 `이 진로로 계획 짜기` 버튼으로 2단계 계획 생성까지 이어진다.
