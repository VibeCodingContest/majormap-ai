...
기준: `C:\VibeCoding_Test\majormap-ai` canonical repo의 현재 코드 상태
...
- 계획 옵션과 계획 결과는 선택한 결과 카드 바로 아래에서 인라인으로 열린다.
- 옵션 선택 후 `POST /api/plan`을 호출한다.
- 학기별 추천 과목, 남은 학점 교양 보완 안내, deferred 과목을 표시한다.
...
- `npm run lint` 통과
- `npm run build` 통과
- Next.js 빌드 결과에서 `/api/recommend`, `/api/plan`, `/api/explain`, `/recommend` 라우트가 확인된다.
...
- canonical repo에 실제로 2단계 planning 코드가 복구됨
...
live URL에서 A/B/C 시나리오를 실제 클릭으로 한 번 더 검증하고, 제출 문서 최종본에 URL과 체크 상태를 반영한다.
